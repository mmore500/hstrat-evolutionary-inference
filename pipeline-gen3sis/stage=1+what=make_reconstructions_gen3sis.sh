...
for try in {0..9}; do
wget "https://osf.io/h3tzf/download" -O "${BATCH_PATH}/data.tar.gz" && break
...
def reconstruct_one(
  ...
  template_df = hstrat_aux.alifestd_coerce_chronological_consistency(
    template_df, mutate=True
  )
