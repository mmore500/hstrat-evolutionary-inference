...
PREV_STAGE_PATH="${HOME}/scratch/data/hstrat-evolutionary-inference/runmode=${RUNMODE}/stage=1+what=make_reconstructions_gen3sis/"
echo "PREV_STAGE_PATH ${PREV_STAGE_PATH}"

STAGE_PATH="${HOME}/scratch/data/hstrat-evolutionary-inference/runmode=${RUNMODE}/stage=2+what=compute_phylometrics_gen3sis/"
...
globbed_true_phylogeny_paths = [
    *glob.glob(
        "${PREV_STAGE_PATH}/latest/data/**/phy.nex",
        recursive=True,
    )
]
logging.info(f"""{
...
def analyze_one(a: str, phylogeny_path: str) -> pd.DataFrame:
  phylogeny_path = phylogeny_path.replace("//", "/")
  ...
  def get_max_origin_time():
    path = phylogeny_path
    if phylogeny_path.endswith(".nex"):
      phylogeny_df = apc.RosettaTree.from_nexus(
        pathlib.Path(path),
      ).as_alife
      phylogeny_df = hstrat_aux.alifestd_to_working_format(phylogeny_df)

      # Gen3sis sometimes produces negative branch lengths... set them to 0
      # https://github.com/project-gen3sis/R-package/issues/70
      phylogeny_df = hstrat_aux.alifestd_coerce_chronological_consistency(
    ...
    return pd.read_csv(path)["origin_time"].fillna(0).max()
  ...
  def do_load_from_file() -> None:
    path = phylogeny_path
    if path.endswith(".nex"):
      phylogeny_df = apc.RosettaTree.from_nexus(
        pathlib.Path(path),
      ).as_alife
      phylogeny_df = hstrat_aux.alifestd_to_working_format(phylogeny_df)
